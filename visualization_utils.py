...
def draw_bounding_box_on_image_array(latest_count, current_frame_number, image,
                                     ymin,
                                     xmin,
                                     ymax,
                                     xmax,
  ...
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  is_vehicle_detected, csv_line, update_csv, inoutdir = draw_bounding_box_on_image(latest_count, current_frame_number,image_pil, ymin, xmin, ymax, xmax, color,
                             thickness, display_str_list,
                             use_normalized_coordinates)
  np.copyto(image, np.array(image_pil))
  return is_vehicle_detected, csv_line, update_csv, inoutdir

def draw_bounding_box_on_image(latest_count, current_frame_number,image,
                               ymin,
                               xmin,
                               ymax,
                               xmax,
  ...
  if(x_axis[0] == 1):
    predicted_direction, is_vehicle_detected, update_csv, inoutdir= object_counter_x_axis.count_objects_x_axis(latest_count, top, bottom, right, left, detected_vehicle_image, ROI_POSITION[0], ROI_POSITION[0]+DEVIATION[0], ROI_POSITION[0]+(DEVIATION[0]*2), DEVIATION[0])
...
def visualize_boxes_and_labels_on_image_array_x_axis(current_frame_number,
  ...
  for box, color in box_to_color_map.items():
    ...
    if ((targeted_objects != None) and (targeted_objects in display_str_list[0])):
            ...
            is_vehicle_detected, csv_line, update_csv, inoutdir = draw_bounding_box_on_image_array(latest_count, current_frame_number,
                image,
                ymin,
                xmin,
                ymax,
  ...
  if(mode == 1):
    ...
    return counter, csv_line_util, counting_mode, inoutdir
